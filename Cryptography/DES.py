"""
This is a pure python implementation of the Data Encryption Standard 
DES with support for multiple modes of operation

Disclaimer !!
The following GItHub Repos. where used in this implemenation...
[1] https://gist.github.com/BlackRabbit-github
[2] https://github.com/dkushagra/DES-Python

"""
################################################################################################
# Class: ciphers and deciphers the plaintext using the DES block cipher
# internal parameters:
...
# Important functions:
#       encrypt_block: encrypt one block of size 64 bits or 8 bytes
#       encrypt: encrypt any stream e.g. arbitrary length text or image using DES
################################################################################################
...
class DES:
    ...
    def __init__(self,master_key):
        assert len(master_key) == 8
        utils.strToBytes(master_key)

        self.L = [] 
        self.R = []
        self.subKeys = [ [0] * 48 ] * 16	# 16 48-bit keys (K1 - K16) (subkeys)
        self.final = []

        master_key = self._key_schedule(master_key)
           
    def _key_schedule(self, master_key):
        key = self.__permutate(self.__pc1, self.__String_to_BitList(master_key) )
        i = 0
        ...
        while i < 16:
            ...
            self.subKeys[i] = self.__permutate(self.__pc2, self.L + self.R)

            i += 1

    def encrypt_block(self, plaintext):
        plaintext = self.__String_to_BitList(plaintext)
        ## IP
        plain_state = self.__permutate(self.__ip, plaintext)
        
        # left and Right 
        self.L = plain_state[:32]
        self.R = plain_state[32:]
        ...
        for i in range(16) :
            ...
            # Permutate R[i - 1] to start creating R[i] (E-box)
            self.R = self.__permutate(self.__expansion_table, self.R)

            # xor with the key
            self.R = list(map(lambda x, y: x ^ y, self.R, self.subKeys[i]))

            #split each 6 bit to apply the 8 s-boxes 
        ...
        # no flip in l;ast ROUND
        self.final = self.__permutate(self.__fp, self.R + self.L)
        return self.__BitList_to_String(self.final)     

    def decrypt_block(self, ciphertext):
        ciphertext = self.__String_to_BitList(ciphertext)
        ## IP
        cipher_state = self.__permutate(self.__ip, ciphertext)
        
        # left and Right 
        self.L = cipher_state[:32]
        self.R = cipher_state[32:]
    ...
    def __permutate(self, table, block):
        """Permutate this block with the specified table (will also drop the 8 additinal bits) """
